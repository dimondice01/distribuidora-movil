...
const ClientMapScreen = () => {
    ...
    const clientsWithLocation = allClients.filter(c => c.location?.latitude && c.location?.longitude);
    ...
    const togglePlannedClient = (client: Client) => {
    ...
    const openNavigationTo = (destination: Client) => {
        ...
        <View style={styles.container}>
            ...
            >
                {clientsWithLocation.map(client => {
                    ...
                    let pinColor = '#3B82F6'; // Azul por defecto para exploración
                    if (isVisited) pinColor = '#9CA3AF'; // Gris para visitado
                    else if (isExecuting) pinColor = '#10B981'; // Verde brillante para parada actual
                    else if (isPlanned) pinColor = '#EF4444'; // Rojo para planificado
                    ...
                    return (
                        <Marker
                            ...
                            // --- CAMBIO: Usamos las propiedades nativas para el info window ---
                            title={client.nombre}
                            description={!isPlanning ? "Toca aquí para ver la ficha" : ""}
                        >
                            {/* El Callout ahora solo captura el evento de clic en la ventana nativa */}
                            <Callout 
                                onPress={() => {
                                    if (!isPlanning) {
                                        router.push({ pathname: '/client-dashboard', params: { clientId: client.id }})
                                    }
                                }}
                            />
                        </Marker>
            ...
            {isNavigating && currentClient && (
                 <View style={styles.routePanel}>
                    ...
                    <TouchableOpacity style={styles.mainActionButton} onPress={() => router.push({ pathname: '/client-dashboard', params: { clientId: currentClient.id } })}>
