...
interface Invoice { id: string; clienteId: string; clienteNombre: string; clienteDireccion: string; totalVenta: number; estadoVisita: 'Pendiente' | 'Pagada' | 'Anulada'; }
interface Route { id: string; nombre: string; facturas: Invoice[]; }
...
const RouteDetailScreen = () => {
    ...
    const route = useMemo(() => routes.find((r: Route) => r.id === routeId), [routes, routeId]);
    const [stops, setStops] = useState<Invoice[]>([]);

    useEffect(() => { if (route) { setStops(route.facturas || []); } }, [route]);
